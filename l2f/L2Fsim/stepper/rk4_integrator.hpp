...
public:
    ...
    std::string file_name2 = "data/energy.txt";

	/* methods */
	/** Constructor */
	rk4_integrator(double _dt=.001) :
        dt(_dt) {
        time = 0.;
    }
        ...
        double period) override
    {
        ...
        save_state_into_file(file_name1,concat,time);
        save_energy_into_file(file_name2,time,period,obs_old,obs);
        */

        /// 3 - Apply the command to the aircraft i.e. modify its state attribute
        ac.apply_command();

        /// 4 - Perform Euler update for a time period of 'period'
        for(int n=0; n<period/dt; ++n) {
            rk4_update(ac,fz,time,dt);
            time += dt;
        }
